...
contract RFYDVault is ERC20, ERC20Detailed, Ownable {
    ...
    address
        private constant ADDRESS_VAULT = ------; // DAI vault
    ...
    function deposit(uint256 _amount) public {
        ...
        uint256 _amountInVaultShares = _totalVaultBalanceAfter.sub(
            _totalVaultBalanceBefore
        );

        balancesToken[msg.sender] = balancesToken[msg.sender].add(_amount);
        balancesVault[msg.sender] = balancesVault[msg.sender].add(
            _amountInVaultShares
        );
    ...
    function withdraw(uint256 _shares) public {
        ...
        uint256 _tokensTransfered = _totalTokenBalanceAfter.sub(
            _totalTokenBalanceBefore
        );

        uint256 _tokensToUser = _shares.mul(balancesToken[msg.sender]).div(
            balancesVault[msg.sender]
        );
        ...
        balancesToken[msg.sender] = balancesToken[msg.sender].sub(
            _tokensToUser
        );
        ...
        returns (bool)
    {
        ...
        if (msg.sender != pool) {
            uint256 _amountInToken = _amount.mul(balancesToken[_sender]).div(
                balancesVault[_sender]
            );

            balancesVault[_sender] = balancesVault[_sender].sub(
                _amount,
                "Vault: transfer amount exceeds balance"
            );
            balancesVault[_recipient] = balancesVault[_recipient].add(_amount);

            balancesToken[_sender] = balancesToken[_sender].sub(
                _amountInToken,
                "Vault: transfer amount exceeds balance"
            );
            balancesToken[_recipient] = balancesToken[_recipient].add(
                _amountInToken
            );
        }
